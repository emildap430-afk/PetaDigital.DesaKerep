          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {docsList.map((imgUrl, idx) => (
                ...
                >
                  <img src={imgUrl} alt={`${itemData.name} ${idx + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
      ...
      <div className="max-w-md mx-auto p-4 space-y-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Informasi sarana & fasilitas yang tersedia di Desa Kerep.
            </p>
          </div>

          <button
            onClick={() => onNavigate({ view: 'dashboard' })}
            className="shrink-0 bg-emerald-900/10 hover:bg-emerald-900/20 text-emerald-950 border border-emerald-800/30 px-2.5 py-1.5 rounded-xl text-[11px] font-bold flex items-center gap-1 transition-colors"
          >
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className="w-full bg-white p-3.5 rounded-2xl border border-slate-200 shadow-2xs hover:border-emerald-500 transition-all flex items-center justify-between text-left group focus:outline-hidden"
              >
