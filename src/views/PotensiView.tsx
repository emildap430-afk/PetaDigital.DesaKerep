          ...
          <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-5">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {docsList.map((imgUrl, idx) => (
                  <button
                    key={idx}
                    onClick={() => setSelectedImage(getAssetUrl(imgUrl))}
            ...
            <div className="space-y-2 pt-1">
              ...
              >
                ...
                <span>Kembali ke {parentPotensi.items ? `${parentPotensi.items.length} Dusun` : 'Daftar Dusun'} {parentPotensi.subtitle}</span>
              </button>
        ...
        <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              {parentPotensi.id === 'budaya'
                ? 'Pelestarian tradisi budaya dan kesenian tradisional di Desa Kerep tersebar di 3 dusun (Cabak, Kerep, dan Balongasem). Pilih dusun di bawah ini untuk melihat detail lengkapnya.'
                : `Pilih dusun di bawah ini untuk melihat detail potensi ${parentPotensi.subtitle.toLowerCase()} dan dokumentasi lengkapnya.`}
            </p>
          </div>
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 max-w-2xl mt-1">
            Pilih sektor potensi di bawah ini untuk melihat persebaran di 3 dusun (Kerep, Balongasem, Cabak) dan dokumentasi lengkapnya.
          </p>
        </div>
            ...
            >
              <div className="space-y-3 w-full">
                <div className="h-44 bg-slate-100 rounded-xl overflow-hidden border border-slate-100 relative">
                  {(p.image || POTENSI_IMAGES[p.id]) ? (
                    <img
                      src={getAssetUrl(p.image) || POTENSI_IMAGES[p.id]}
                      alt={p.title}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                      loading="eager"
                    />
                  ...
                  <div className="absolute top-2.5 left-2.5 bg-[#0b3c2c]/90 backdrop-blur-xs text-amber-300 text-[10px] font-extrabold px-2.5 py-1 rounded-full flex items-center gap-1 border border-amber-300/30">
                    <Sparkles className="w-3 h-3" />
              ...
              <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-emerald-800 w-full">
                <span className="flex items-center gap-1">
                  ...
                  <span className="truncate max-w-[170px]">Komoditas: {p.komoditas}</span>
                </span>
