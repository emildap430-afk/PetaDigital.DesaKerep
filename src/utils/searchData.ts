...
export const getSearchIndex = (): SearchResultItem[] => {
  const items: SearchResultItem[] = [
    ...
    },
    {
      ...
      categoryBadge: 'Profil Desa',
      categoryColor: 'bg-emerald-100 text-emerald-800 border-emerald-200',
      description: 'Batas wilayah, iklim, dan 3 Dusun: Kerep, Balongasem, Cabak.',
      keywords: ['geografis', 'peta', 'lokasi', 'batas', 'luas', 'dusun', 'balongasem', 'cabak'],
    ...
    // --- SUB-ITEM POTENSI PER DUSUN ---
    ...potensiList.flatMap((p) =>
      (p.items || []).map((item) => ({
        id: `pot-sub-${item.id}`,
        title: item.name,
        category: 'Potensi' as const,
        categoryBadge: `${p.subtitle} - ${item.dusun}`,
        categoryColor: 'bg-amber-100 text-amber-800 border-amber-200',
        ...
        keywords: [
          'potensi',
          p.subtitle.toLowerCase(),
          item.dusun.toLowerCase(),
          item.name.toLowerCase(),
          ...(item.komoditas ? item.komoditas.toLowerCase().split(' ') : []),
          ...(item.fungsi ? item.fungsi.map((f) => f.toLowerCase()) : []),
          ...item.description.toLowerCase().split(' ')
        ],
        route: { view: 'potensi-detail' as const, potensiId: p.id, potensiDusunId: item.id }
      }))
    ),
